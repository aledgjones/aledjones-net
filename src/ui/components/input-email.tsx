...
export const InputEmail: FC<Props> = ({ id, className, style, value, label, color, errorColor, required, disabled, onChange }) => {
    ...
    const _onChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (required && !e.target.value || !isEmail(e.target.value)) {
            setError(true);
        } else {
            setError(false);
        }
